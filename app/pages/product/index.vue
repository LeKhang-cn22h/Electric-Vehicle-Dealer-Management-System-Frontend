...
const filteredCars = computed<Vehicle[]>(() => {
    ...
    return list.filter((car: Vehicle) => {
        const matchesSearch = !keyword.value || car.name?.toLowerCase().includes(keyword.value.toLowerCase());

        const matchesModel = !filters.value.model || car.model?.toLowerCase().includes(filters.value.model.toLowerCase());
...
<template>
    <div class="p-6">
        ...
        <div v-if="compareList.length > 0" class="mb-4 p-3 bg-blue-50 rounded-lg flex items-center justify-between">
            <span class="text-blue-700 font-medium"> Đã chọn {{ compareList.length }} xe để so sánh </span>
        </div>
        ...
        <div v-else-if="error" class="text-center py-12">
            ...
            <button @click="() => fetchAll()" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                Thử lại
            </button>
        </div>
        ...
        <div v-else-if="filteredCars.length === 0" class="text-center py-12">
            ...
            <button @click="resetAllFilters" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                Xóa bộ lọc
            </button>
        </div>
        ...
        <!-- Modal so sánh -->
        <CompareModal :visible="showCompare" :cars="compareList" @close="closeCompareModal" />
    </div>
</template>
