        ...
        <div class="max-w-4xl mx-auto">
            ...
            <header class="mb-8">
                <div class="flex justify-between items-center">
                    <div>
                        <h1 class="text-2xl font-semibold text-gray-900">Chi tiết hợp đồng #{{ contractId }}</h1>
            ...
            <!-- Nội dung chi tiết -->
            <div v-else-if="contract" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-8">
                ...
                <section>
                    ...
                    <table class="w-full border border-gray-200 rounded-lg">
                        ...
                        <tbody>
                            <tr v-for="item in contract.vehicles" :key="item.id" class="border-t">
                                <td class="px-4 py-2">{{ item.name }}</td>
                                <td class="px-4 py-2">
                                    {{ contract.items.find((i) => i.product_id === item.id)?.quantity || 0 }}
                                </td>
                ...
                <section>
                    ...
                    <p><strong>Phương thức:</strong> {{ contract.paymentMethod === "cash" ? "Tiền mặt" : "Trả góp" }}</p>
                    <p v-if="contract.paymentMethod === 'bank_transfer'">
                        <strong>Đối tác ngân hàng:</strong> {{ contract.order.bank }}
                    </p>
                    <p><strong>Trả trước:</strong> {{ formatCurrency(contract.order.down_payment) }}</p>
                    <p><strong>Kỳ hạn:</strong> {{ contract.order.term }} tháng</p>
                </section>

                <section>
                    <h2 class="text-lg font-semibold text-gray-800 mb-3">Thời hạn hợp đồng</h2>
                    <p>
                ...
                <section class="border-t pt-4">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-2xl font-bold text-blue-600">{{ formatCurrency(contract.order.total_amount) }}</span>
                    </div>
                </section>
            </div>
