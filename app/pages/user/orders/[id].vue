        ...
        <section class="border-t pt-4 mt-6 space-y-2 text-gray-700">
          ...
          >
            ...
            <p v-if="order.term">
              ...
              {{
                formatCurrency(
                  (order.total_amount - (order.down_payment || 0)) / order.term
                )
              }}
              × {{ order.term }} tháng
            </p>
...
const mapOrderToBillPayload = (orderData: any): CreateBillPayload => {
  ...
  return {
    ...
    meta: {
      ...
      is_installment:
        (orderData.paymentMethod ?? orderData.payment_method) === "installment",
      dealer: {
        id: dealer_id,
        name: dealerName,
...
const handleConfirm = async () => {
  ...
  try {
    ...
    errorContract.value =
      err?.data?.message ||
      err?.message ||
      "Có lỗi xảy ra trong quá trình tạo hợp đồng / hóa đơn";
  } finally {
