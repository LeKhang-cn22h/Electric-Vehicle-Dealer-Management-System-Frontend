        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div v-if="currentStep === 1" class="space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <OrderPromotionSelect v-model:promotions="orderData.appliedPromotions" :items="orderData.items" />
                </div>

                <!-- Payment Section -->
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <h2 class="text-lg font-semibold text-gray-900 mb-4">Khuyến mãi áp dụng</h2>
                    <OrderPaymentMethod v-model:payment="orderData.payment" :total="orderTotal" />
                </div>
                <!-- Notes Section -->
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <NuxtLink to="/dealer_manager/payments/vnpay-demo" class="nav-item"> Test thanh toán VNPay </NuxtLink>
            ...
            <!-- Step 2: Xác nhận -->
            <div v-else>
                <OrderSummary :order-data="orderData" :order-total="total" @back="currentStep = 1" @submit="createOrder" />
            </div>
        </div>
    </div>
</template>
...
// --- Computed totals ---
const subtotal = computed(() => orderData.items.reduce((sum, item) => sum + (item.quantity || 0) * (item.unitPrice || 0), 0));
const promoDiscount = computed(() => orderData.appliedPromotions.reduce((sum, promo) => sum + promo.discountAmount, 0));
const percentDiscount = computed(() => (subtotal.value * (orderData.discountPercent || 0)) / 100);
