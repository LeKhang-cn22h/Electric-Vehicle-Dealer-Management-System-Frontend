<template>
   <form @submit.prevent="SendFB" class ="w-full max-w-2xl bg-white shadow-lg rounded-2xl p-8">
    ...
    <div class="mb-4">
        ...
        <input v-model="form.email" type="email" placeholder="Nhập email của bạn" class="border border-gray w-full rounded-xl px-4 py-2">
    ...
    <div class="mb-4">
        ...
        <input type="text" placeholder="Nhập tiêu đề phản hồi/khiếu nại" class="border border-gray px-4 py-2 rounded-xl w-full ">
    </div>
    <div class="mb-">
        <label class="block font-medium mb-2">Nội dung</label>
        <textarea placeholder="Mô tả nội dung phản hồi hoặc khiếu nại của bạn ..." class="w-full border border-gray rounded-xl px-2 py-4">
    ...
    <p v-if="thongBao" class="text-green-600 font-semibold mt-4 text-center">{{ thongBao }}</p>
   </form>
</template>
...
<script setup lang="ts">
    ...
    function SendFB(){
        ...
        thongBao.value="Cảm ơn bạn đã gửi phản hồi Chúng tôi sẽ xử lý trong thời gian sớm nhất.";
        emit('submitted');
        setTimeout(()=>{
            thongBao.value="";
        },3000);
        form.value={hoTen:"", email:"", daiLy:"", tieuDe:"", noiDung:""};
