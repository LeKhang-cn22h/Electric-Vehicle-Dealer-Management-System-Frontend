...
export function useOrders() {
  ...
  const attachInvoice = async (orderId: string, invoiceId: string) => {
    loading.value = true;
    error.value = null;
    try {
      const res = await OrderService.attachInvoice(orderId, invoiceId);
      console.log("[useOrders] attachInvoice:", res);
      return res;
    } catch (e: any) {
      error.value = e.message || "Error attaching invoice to order";
      console.error("[useOrders] attachInvoice error:", e);
      throw e;
    } finally {
      loading.value = false;
    }
